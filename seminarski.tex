...
Pirs nije verovao u jasnu granicu izme\dj u ta\v cnog i neta\v cnog, ve\' c u to da je \v zivot kontinuum.
...
\\
\\
U nastavku su predstavljene 2 metode za pobolj\v savanje kvaliteta slike zasnovane na fazi logici: jedna za slike u boji, druga za crno-bele slike.
...
prevo\dj enje nefazi ulaza (slika $X$) u njen odgovaraju\' ci fazi oblik.
...
\begin{align*}
  ...
  y_{ij} = x_{min} + \bar\mu \cdot (x_{max} - x_{min})
\end{align*}
...
\begin{align*}
  \mu_{ij} = \Bigg( 1 + \frac{x_{max} - x_{ij}}{Fd}\Bigg)^{-Fe} \\ \\
  0\leq i < M, 0\leq j < N
\end{align*}
...
$Fe$ je eksponencijalna fazi konstanta (eng. exponential fuzzifier), koja uzima pozitivne vrednosti. \\
Eksperimentalno je pokazano da su dobro izabrane vrednosti za Fe 1 i 2. \\
U na\v soj implementaciji 
\begin{align*}
  Fe = 2\\
\end{align*}
$Fd$ je koli\v cni\v cka fazi konstanta (eng. denominational fuzzifier), koja uzima pozitivne vrednosti. \\
\begin{align*}
...
\begin{align*}
  y_{ij} = \mu^{-1}(\bar\mu_{ij})\\
  y_{ij} = x_{max} - Fd \cdot (\bar\mu_{ij})^{-Fe} + Fd\\
\end{align*}
